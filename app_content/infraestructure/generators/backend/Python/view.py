import os
from app_content.application.interface.backend.view import View
from app_content.application.interface.generator import Generator

class PythonView(View, Generator):
    def execute(self,):
        ''' Generate the files for the model '''
        nombre = f"{self.model.nombre[0].capitalize()}{self.model.nombre[1:]}"
        path = os.path.join(self.basepath, "presentation", "views")
        os.makedirs(path, exist_ok=True)

        file_path = os.path.join(path, f"{self.model.nombre}_view.py")
        try:
            with open(file_path, "w") as f:
                f.write(f"from {self.basepath.replace('/','.')}.application.usecases.{self.model.nombre}_usecase import {nombre}UseCase\n")
                f.write(f"from {self.basepath.replace('/','.')}.infraestructure.repositories.django{self.model.nombre}_repository import Django{nombre}Repository\n")
    ...
    def post(self,)->str:
        ...
        return (
            ...
            f"            return Response(obj.__dict__, status=status.HTTP_201_CREATED)\n"
            f"        except Exception as e:\n"
    ...
    def get(self, )->str:
        ...
        return (
            ...
            f"            else:\n"
            f"                data = obj.__dict__\n"
            f"            return Response(data, status=status.HTTP_200_OK)\n"
            f"        except Exception as e:\n"
    ...
    def put(self,)->str:
        ...
        return (
            ...
            f"            return Response(obj.__dict__, status=status.HTTP_200_OK)\n"
            f"        except Exception as e:\n"
            f"            return Response({{'error': str(e)}}, status=status.HTTP_400_BAD_REQUEST)\n"
        )
