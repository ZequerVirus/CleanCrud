import os
from app_content.application.interface.backend.repositoryimpl import RepositoryImpl
from app_content.application.interface.generator import Generator

class PythonRepositoryImpl(RepositoryImpl, Generator):

    def execute(self,):
        ''' Generate the files for the model '''
        nombre = f"{self.model.nombre[0].capitalize()}{self.model.nombre[1:]}"
        path = os.path.join(self.basepath, "infraestructure", "repositories")
        os.makedirs(path, exist_ok=True)

        file_path = os.path.join(path, f"django{self.model.nombre}_repository.py")
        try:
            with open(file_path, "w") as f:
                ...
                f.write(f"from {self.basepath.replace('/','.')}.models import {self.model.nombre}\n")
    ...
    def map(self,)->str:
        ...
        return (
            ...
            f"{",\n".join([f"                {field.nombre}=instance.{field.nombre}" for field in self.model.fields])},\n"
    ...
    def save(self,)->str:
        ...
        return (
            ...
            f"{"\n".join([f'            instance.{field.nombre} = obj.{field.nombre}' for field in self.model.fields if field.nombre != 'id'])}\n" 
