...
class ReactHook:
    ...
    def __load(self, model: ModelEntity, nombre: str):
        return (
            ...
            f"    setState({{ ...state, loading: true, message: '' }});\n"
            f"    try {{\n"
            ...
            f"        setState({{ ...state, items: objs, filtered: objs,}});\n"
            f"    }} catch (error) {{\n"
            f"      setState({{ ...state, message: `Error obteniendo {model.nombre}: ${{error}}` }});\n"
            f"    }} finally {{\n"
            f"      setState({{ ...state, loading: false }});\n"
            f"    }}\n"
            f"  }};\n"
        )
    ...
    def __create(self, model: ModelEntity, nombre: str):
        return (
            ...
            f"      }}\n"
            f"      setState({{ ...state, message: \"{nombre} creado\"}});\n"
            f"    }} catch (error) {{\n"
    ...
    def __update(self, model: ModelEntity, nombre:str):
        return (
            ...
            f"      }}\n"
            f"      setState({{ ...state, message: \"{nombre} actualizado\",}});\n"
            f"    }} catch (error) {{\n"
    ...
    def __delete(self, model: ModelEntity, nombre:str):
        return (
            ...
            f"      }}\n"
            f"      setState({{ ...state, message: \"{nombre} eliminado\",}});\n"
            f"    }} catch (error) {{\n"
    ...
    def __search(self, model: ModelEntity, nombre:str):
        return (
            ...
            f"    if (!value){{\n"
            f"      setState({{ ...state, filtered: state.items }});\n"
            f"      return;\n"
            f"    }}\n"
            f"    const lower=value.toLowerCase();\n"
            f"    const filtered = state.items.filter((item: {nombre}Entity) => {{\n"
            f"      return {(' ||\n').join(['{field.nombre}.toLowerCase().includes(lower)' for field in model.fields])};\n"
            f"    }});\n"
            f"    setState({{ ...state, filtered: filtered }});\n"
            f"  }};\n"
        )