...
class ReactView(Generator):
    def __init__(self):
        pass

    def execute(self,):
        '''Genera la vista React para el modelo dado'''
        nombre = f"{self.model.nombre[0].upper()}{self.model.nombre[1:]}"
        path = os.path.join(self.basepath, "presentation", "views", f"{nombre}")
        os.makedirs(path, exist_ok=True)
        filepath = os.path.join(path, f"{self.model.nombre}_view.tsx")
        try:
            with open(filepath, "w") as f:
                f.write(self.__imports())
                f.write(self.__component())
    ...
    def __imports(self, )->str:
        ...
        return (
            ...
            f"import {nombre}Table from './{self.model.nombre}_table';\n"
            f"import {{ useState }} from 'react';\n"
            f"import {{ {nombre}Hook }} from './{self.model.nombre}_hook';\n\n"
        )

    def __component(self, )->str:
        nombre = f"{self.model.nombre[0].capitalize()}{self.model.nombre[1:]}"
        return (
            f"function {nombre}() {{\n"
            f"  const {{\n"
